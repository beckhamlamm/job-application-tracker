...
test('blocks IPv6 loopback, mapped IPv4, and private addresses', () => {
  for (const url of ['http://[::1]/', 'http://[::ffff:127.0.0.1]/', 'http://[fc00::1]/', 'http://127.1/', 'http://10.0.0.1/']) {
...
test('pins a public address and blocks a redirect into localhost', async () => {
  ...
  await assert.rejects(publicFetch('https://example.com', {
    ...
    requestImpl: (_url, options, callback) => {
      calls++;
      options.lookup('example.com', {}, (err, address) => { assert.equal(err, null); assert.equal(address, '93.184.216.34'); });
      const req = new EventEmitter();
      req.end = () => {
        const res = new PassThrough(); res.statusCode = 302; res.headers = { location: 'http://[::1]/' };
