...
test('malformed JSON does not prevent microdata fallback', () => {
  const html =
    '<script type="application/ld+json">{broken</script><div itemtype="https://schema.org/JobPosting"><span itemprop="title">Engineer</span></div>';
...
test('multiple unmatched structured jobs do not select an arbitrary posting', () => {
  const html =
    '<script type="application/ld+json">[{"@type":"JobPosting","url":"/jobs/1"},{"@type":"JobPosting","url":"/jobs/2"}]</script>';
...
test('inline organization fields take precedence over referenced fields', () => {
  ...
  assert.equal(
    getStructuredJob(html, 'https://example.org/job').hiringOrganization[0].name,
    'Local Studio',
  );
});
