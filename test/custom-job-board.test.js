...
test('Helsing returns company and role and preserves the submitted URL', async () => {
  const result = await resolveCustomJobBoard(url, { fetchImpl: async (endpoint) => ({ ok: true, json: async () => endpoint.endsWith('/4976480101')
    ? { id: 4976480101, title: 'Junior Software Engineer ', updated_at: '2026-09-15' } : { name: 'Helsing' } }) });
...
test('unrecognized domains do not trigger guessed ATS requests', async () => {
  assert.equal(await resolveCustomJobBoard('https://example.com/jobs/123?gh_jid=123', { fetchImpl: () => assert.fail() }), null);
});
...
test('conflicting job IDs and closed jobs are reported', async () => {
  ...
  await assert.rejects(resolveCustomJobBoard(url, { fetchImpl: async () => ({ ok: false }) }), /may have closed/);
});
