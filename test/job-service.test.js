...
const html = '<script type="application/ld+json">{"@type":"JobPosting","title":"Engineer","datePosted":"2026-01-15","hiringOrganization":{"name":"Example Studio"}}</script>';
...
test('service combines page fields and company evidence without exposing intermediate data', async () => {
  ...
  assert.deepEqual(await parse(url), {
    url, role: 'Engineer', datePosted: '2026-01-15', company: 'Example Studio',
    companySource: 'JobPosting structured data', companyConfidence: 'high',
  });
});
...
test('verified board data can resolve a blocked page using the injected fetch dependency', async () => {
  const responses = [page('<title>Access Denied</title>', false),
    { ok: true, json: async () => ({ id: 123, title: 'Designer', absolute_url: url }) },
    { ok: true, json: async () => ({ name: 'Example Studio' }) }];
  const signals = [];
  const parse = createJobService({ fetchPage: async (_, { signal }) => {
    signals.push(signal);
...
test('unresolved blocked pages never enter page-field parsing', async () => {
  for (const response of [page('', false), page('<title>Vercel Security Checkpoint</title>')]) {
    const parse = createJobService({ fetchPage: async () => response, parsePage: () => assert.fail('must not parse blocked content') });
...
test('parsing strategies can be replaced without changing orchestration', async () => {
  const parsed = Object.freeze({ url, role: 'Designer', structuredJob: { title: 'Designer' }, pageCompany: 'Studio' });
  const fetchPage = async () => page();
  const parse = createJobService({ fetchPage, resolveBoard: async () => null, parsePage: () => parsed,
...
test('service propagates fetch failures and aborts slow fetches', async () => {
  const failed = createJobService({ fetchPage: async () => { throw new Error('Unavailable'); } });
  await assert.rejects(failed(url), /Unavailable/);
  const slow = createJobService({ timeoutMs: 5, fetchPage: (_, { signal }) => new Promise((resolve, reject) => {
