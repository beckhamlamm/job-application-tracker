...
test('a hosted posting can identify its employer in text when structured organization is blank', async () => {
  ...
  const html = `<script type="application/ld+json">${JSON.stringify({
    ...
    description:
      'Our teams use Example Tools products. Northstar Labs is an Equal Opportunity Employer.',
  ...
  const company = await resolveCompany({
    html,
    url,
    job: parsed.structuredJob,
    fetchImpl: () => assert.fail('page evidence needs no network'),
  });
  ...
  assert.equal(
    (
      await resolve({
        ...
        description: 'Northstar Labs is an Equal Opportunity Employer.',
      })
    ).name,
    'Example Studio',
  );
  for (const description of [
    'Experience with Example Tools and Workday products.',
    'The company is an Equal Opportunity Employer.',
    'Northstar Labs is an Equal Opportunity Employer. Example Studio is an Equal Opportunity Employer.',
  ]) {
...
test('a verified embedded board preserves the URL and does not treat an update date as a posting date', async () => {
  ...
  const endpoints = [];
  const result = await resolveCustomJobBoard(url, {
    fetchImpl: async (endpoint) => {
      endpoints.push(endpoint);
      return {
        ok: true,
        json: async () =>
          endpoint.endsWith('/jobs/123')
            ? {
                id: 123,
                title: ' Software Engineer ',
                absolute_url: 'https://example.org/jobs/123',
...
test('a listing with multiple embedded job links is not treated as one posting', () => {
  assert.deepEqual(
    discoverBoards(
      ...
      '<a href="https://boards.greenhouse.io/example/jobs/1">One</a><a href="https://boards.greenhouse.io/example/jobs/2">Two</a>',
    ),
    [],
  );
});
...
test('custom HTML heading outranks a generic site title', async () => {
  const html =
    '<title>Careers</title><main><h1>Senior <span>Engineer</span></h1></main><meta content="O\'Brien Labs" property="og:site_name">';
  ...
  assert.equal(
    (await resolveCompany({ html, url: parsed.url, job: parsed.structuredJob })).name,
    "O'Brien Labs",
  );
});
test('JSON-LD references resolve across separate scripts', () => {
  const html =
    '<script type="application/ld+json">{"@type":"JobPosting","title":"Designer","hiringOrganization":{"@id":"#org"}}</script><script type="application/ld+json">{"@id":"#org","@type":"Organization","name":"Studio"}</script>';
  assert.equal(
    parseJobPage(html, 'https://studio.test/job').structuredJob.hiringOrganization.name,
    'Studio',
  );
});
...
test('embedded JSON from a JS application supports exact job selection', () => {
  const html =
    '<script type="application/json">{"jobs":[{"@type":"JobPosting","title":"Wrong","url":"/jobs/1"},{"@type":"JobPosting","title":"Correct","url":"/jobs/2"}]}</script>';
...
test('discovers a board with a different name from an iframe on any domain', async () => {
  const result = await resolveCustomJobBoard('https://example.org/careers/engineer', {
    ...
    fetchImpl: async (url) => ({
      ok: true,
      json: async () =>
        url.endsWith('/123') ? { id: 123, title: 'Engineer' } : { name: 'Example Org' },
    }),
  });
  ...
  const result = await resolveCustomJobBoard('https://example.org/jobs/123?gh_jid=123', {
    fetchImpl: async () => ({
      ...
      json: async () => ({
        id: 123,
        title: 'Wrong',
        absolute_url: 'https://unrelated.com/jobs/123',
      }),
    }),
  });
