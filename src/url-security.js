...
function isPrivateHost(hostname) {
  ...
  if (net.isIP(host) === 6) {
    ...
    return !/^[23][0-9a-f]{3}:/.test(host) || /^2001:(?:0:|2:|10:|20:|db8:)/.test(host) || host.startsWith('2002:');
  }
  if (net.isIP(host) === 4) {
...
function validatePublicUrl(input) {
  ...
  if (!['http:', 'https:'].includes(target.protocol) || target.username || target.password || isPrivateHost(target.hostname)) {
