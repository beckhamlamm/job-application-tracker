...
function objects(value, output = [], depth = 0) {
  if (!value || typeof value !== 'object' || depth > MAX_OBJECT_DEPTH) {
    return output;
  }
  output.push(value);
  for (const child of Object.values(value)) {
    objects(child, output, depth + 1);
  }
  return output;
}
...
function hasType(value, type) {
  return [value?.['@type']]
    .flat()
    .some((item) => typeof item === 'string' && item.split(/[/#]/).pop() === type);
}
...
function readJsonNodes($) {
  ...
  $('script').each((_, el) => {
    ...
    try {
      objects(JSON.parse($(el).text()), nodes);
    } catch {
...
function selectJsonJob(nodes, url) {
  ...
  const selected =
    jobs.find((job) => samePage(job.url || job.mainEntityOfPage?.['@id'], url)) ||
  ...
  const resolve = (org) =>
    org?.['@id']
      ? { ...nodes.find((node) => node['@id'] === org['@id'] && node.name), ...org }
      : org;
  return {
    ...selected,
    hiringOrganization: Array.isArray(selected.hiringOrganization)
      ? selected.hiringOrganization.map(resolve)
      : resolve(selected.hiringOrganization),
  };
}

function readMicrodataJob($) {
  const scopes = $('[itemtype], [typeof]').filter((_, el) =>
    /(?:^|[\s/#:])JobPosting$/.test($(el).attr('itemtype') || $(el).attr('typeof') || ''),
  );
  if (scopes.length !== 1) {
    return null;
  }
  ...
  return {
    '@type': 'JobPosting',
    title: value(field(scope, 'title')),
    datePosted: value(field(scope, 'datePosted')),
    description: value(field(scope, 'description')),
    hiringOrganization: { name: value(field(org, 'name')) || value(org) },
  };
}
