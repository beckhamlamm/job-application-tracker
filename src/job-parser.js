...
function getMetaContent(html, key) {
  ...
  return (
    $('meta')
      .filter((_, el) => [$(el).attr('name'), $(el).attr('property')].includes(key))
      .first()
      .attr('content') || ''
  );
}
    ...
    try {
      for (const node of objects(JSON.parse($(el).text()))) {
        if (hasType(node, 'Organization') && node.name && node.url) {
          try {
            if (getDomain(new URL(node.url, url).hostname) === getDomain(new URL(url).hostname)) {
              pageCompany = decodeHtml(node.name);
            }
          } catch {
            /* Invalid organization URLs are not usable company evidence. */
          }
        }
      }
    } catch {
      /* Optional malformed JSON must not prevent HTML fallback extraction. */
    }
  });
  $('script, style, nav, footer, [hidden], [aria-hidden="true"]').remove();
  const heading = $('main h1, article h1').first().text() || $('h1').first().text();
  const generic =
    /^(?:careers?|jobs?|join (?:us|our team)|open (?:roles|positions)|opportunities|apply(?: now)?)$/i;
  let role = decodeHtml(job?.title || job?.name);
  if (!role) {
    role =
      [heading, getMetaContent(html, 'og:title'), $('title').text()]
        .map(decodeHtml)
        .find(
          (text) =>
            text &&
            !generic.test(text) &&
            !/security checkpoint|access denied|verify.*browser/i.test(text),
        ) || '';
  }
  const brand = getMetaContent(html, 'og:site_name');
  if (brand && role.endsWith(` | ${brand}`)) {
    role = role.slice(0, -brand.length - 3).trim();
  }
