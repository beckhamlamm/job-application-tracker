...
function createJobService({
  ...
  return async function parseJob(input) {
    ...
    try {
      ...
      const company = await resolveEmployer({ ...options, html, url: response.url, job: structuredJob, pageCompany });
      return { ...fields, company: company.name, companySource: company.source, companyConfidence: company.confidence };
    } finally {
